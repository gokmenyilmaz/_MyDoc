"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var MyW3CarouselComponent = (function () {
    function MyW3CarouselComponent() {
        this.slideItems = [];
        this.indexItem = 0;
    }
    MyW3CarouselComponent.prototype.ngOnInit = function () {
        var _this = this;
        setInterval(function () { _this.carousel(_this.indexItem += 1); }, this.saniye);
        console.log(this.saniye);
    };
    MyW3CarouselComponent.prototype.plusDivs = function (i) {
        this.carousel(this.indexItem += i);
    };
    MyW3CarouselComponent.prototype.carousel = function (indexItem) {
        console.log(indexItem);
        if (this.indexItem > this.slideItems.length - 1) {
            this.indexItem = 0;
        }
        if (this.indexItem < 0) {
            this.indexItem = this.slideItems.length - 1;
        }
        for (var _i = 0, _a = this.slideItems; _i < _a.length; _i++) {
            var item = _a[_i];
            item.sitil = "none";
        }
        this.slideItems[this.indexItem].sitil = "block";
    };
    return MyW3CarouselComponent;
}());
__decorate([
    core_1.Input(),
    __metadata("design:type", Array)
], MyW3CarouselComponent.prototype, "slideItems", void 0);
__decorate([
    core_1.Input(),
    __metadata("design:type", Number)
], MyW3CarouselComponent.prototype, "saniye", void 0);
__decorate([
    core_1.Input(),
    __metadata("design:type", String)
], MyW3CarouselComponent.prototype, "testdata", void 0);
__decorate([
    core_1.Input(),
    __metadata("design:type", Boolean)
], MyW3CarouselComponent.prototype, "sadeceMetin", void 0);
MyW3CarouselComponent = __decorate([
    core_1.Component({
        moduleId: module.id,
        selector: "my-w3carousel",
        templateUrl: "carousel.component.html",
        styleUrls: ["carousel.component.css"]
    }),
    __metadata("design:paramtypes", [])
], MyW3CarouselComponent);
exports.MyW3CarouselComponent = MyW3CarouselComponent;
//# sourceMappingURL=carousel.component.js.map